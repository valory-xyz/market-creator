...
def mock_contract_api_response(
    performative: ContractApiMessage.Performative, body: Dict[str, Any]
) -> MagicMock:
...
def behaviour(monkeypatch):
    ...
    monkeypatch.setattr(
        DummyPrepareTransactionBehaviour, "context", PropertyMock(return_value=mock_ctx)
    )
...
def test_calculate_time_parameters(behaviour):
    ...
    opening_timestamp, timeout_seconds = behaviour._calculate_time_parameters(
        resolution_time=resolution_time, timeout=timeout
    )

    # Expected: resolution_time + 1 day
    expected_opening = int(
        datetime.fromtimestamp(resolution_time + _ONE_DAY).timestamp()
    )
...
@pytest.mark.parametrize(
    "performative, data, expected_result",
    [
        (
            ContractApiMessage.Performative.STATE,
            {"data": "0xASK_QUESTION_TX_DATA"},
            {
                "to": "0xREALITIO_CONTRACT",
                "data": "0xASK_QUESTION_TX_DATA",
                "value": 100,
            },
        ),
...
def test_prepare_ask_question_mstx(
    behaviour, question_data, performative, data, expected_result
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ContractApiMessage.Performative.STATE,
            {"data": "0xPREPARE_CONDITION_TX_DATA"},
            {
                "to": "0xCONDITIONAL_TOKENS",
                "data": "0xPREPARE_CONDITION_TX_DATA",
                "value": 0,
            },
        ),
    ...
    if performative == ContractApiMessage.Performative.STATE:
        ...
        with patch.object(behaviour, "_prepare_prepare_condition_mstx", wrapped_method):
            ...
            gen = behaviour._prepare_prepare_condition_mstx(
                question_id=question_id, outcome_slot_count=2
            )
...
@pytest.mark.parametrize(
    "performative, body, expected_result",
    [
        (
            ContractApiMessage.Performative.STATE,
            {"data": "0xCREATE_FPMM_TX_DATA", "value": 1500},
            {
                "to": "0xFPMM_DETERMINISTIC_FACTORY_CONTRACT",
                "data": "0xCREATE_FPMM_TX_DATA",
                "value": 0,
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "to": "0xCOLLATERAL_TOKENS_CONTRACT",
                "data": "0xAPPROVE_TX_DATA",
                "value": 0,
            },
        ),
...
def test_async_act_success(behaviour):
    ...
    behaviour._get_approve_tx = MagicMock(side_effect=[gen_side_effect(approve_tx)])

    behaviour._to_multisend = MagicMock(side_effect=[gen_side_effect(tx_hash)])
    ...
    behaviour._prepare_prepare_condition_mstx.assert_called_once_with(
...
def test_async_act_question_id_failure(behaviour, monkeypatch):
    ...
    monkeypatch.setattr(
        behaviour, "_prepare_ask_question_mstx", prepare_ask_question_mock
    )
...
@pytest.mark.parametrize(
    "failure_point, should_call_prepare_condition, should_call_calculate_condition, should_call_create_fpmm, should_call_get_approve, should_call_to_multisend",
    [
...
def test_async_act_various_failure_points(
    ...
    def custom_async_act():
        ...
        resolution_time = int(
            behaviour.synchronized_data.approved_question_data["resolution_time"]
        )
        opening_timestamp, timeout = calculate_time_mock(
            resolution_time=resolution_time, timeout=behaviour.params.market_timeout
        )
            ...
            if failure_point not in ["create_fpmm"]:
                ...
                if failure_point not in ["get_approve"]:
                    ...
                    if failure_point not in ["to_multisend"]:
                        yield from send_a2a_mock(
                            PrepareTransactionPayload(
                                behaviour.context.agent_address, tx_hash
                            )
                        )
    ...
    calculate_question_id_mock = MagicMock(
        side_effect=[gen_side_effect("0xQUESTION_ID")]
    )
    prepare_ask_question_mock = MagicMock(
        side_effect=[gen_side_effect({"to": "0x", "data": "0x", "value": 0})]
    )
    ...
    if failure_point == "prepare_condition":
        ...
        prepare_condition_mock = MagicMock(
            side_effect=[gen_side_effect({"to": "0x", "data": "0x", "value": 0})]
        )
    ...
    if failure_point == "calculate_condition":
        ...
        calculate_condition_mock = MagicMock(
            side_effect=[gen_side_effect("0xCONDITION_ID")]
        )

    create_fpmm_mock = MagicMock(
        side_effect=[
            gen_side_effect(
                {"to": "0x", "data": "0x", "value": 0, "approval_amount": 100}
                if failure_point != "create_fpmm"
                else None
            )
        ]
    )

    get_approve_mock = MagicMock(
        side_effect=[
            gen_side_effect(
                {"to": "0x", "data": "0x", "value": 0}
                if failure_point != "get_approve"
                else None
            )
        ]
    )

    to_multisend_mock = MagicMock(
        side_effect=[
            gen_side_effect("0xTX_HASH" if failure_point != "to_multisend" else None)
        ]
    )
