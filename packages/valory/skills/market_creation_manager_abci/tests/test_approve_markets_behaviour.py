...
from dynamic_module_7415541777513949010 import ApproveMarketsBehaviour
...
def mock_http_response(status_code, body=None):
    ...
    http_response.body = json.dumps(body or {}).encode("utf-8") if body is not None else b"{}"
...
class DummyApproveMarketsBehaviour(ApproveMarketsBehaviour):
    ...
    def __init__(self, is_sender=True):
        ...
        self.last_synced_timestamp = 1713900000  # Example timestamp
...
# --------------------------------------------------------------------------- #
@pytest.fixture
def sender_behaviour(monkeypatch):
    """Provide a behaviour instance that is the sender with mocked context."""
    beh = DummyApproveMarketsBehaviour(is_sender=True)
    ctx_patch = patch.object(DummyApproveMarketsBehaviour, "context", new_callable=PropertyMock)
...
@pytest.fixture
def non_sender_behaviour(monkeypatch):
...
def mock_mech_tool():
    ...
    with patch("packages.valory.skills.market_creation_manager_abci.propose_questions") as mock_tool:
...
def test_sender_act_with_http_errors(sender_behaviour, mock_mech_tool):
    ...
    http_responses = [
        mock_http_response(200, {"result": "market_proposed"}),  # Propose market 1 success
        ...
        mock_http_response(200, {"result": "market_proposed"}),  # Propose market 2 success
        mock_http_response(200, {"result": "market_approved"}),  # Approve market 2 success
        mock_http_response(200, {"result": "market_updated"}),   # Update market 2 success
    ]
