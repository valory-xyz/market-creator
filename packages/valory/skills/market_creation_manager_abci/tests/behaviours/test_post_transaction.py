...
class TestPostTransactionBehaviour:
    ...
    def test_get_payload_fpmm_remove_liquidity_submitter(self) -> None:
        """Test get_payload when tx_submitter is FpmmRemoveLiquidityRound."""
        self.behaviour.synchronized_data.settled_tx_hash = "0xabc"
        ...
        assert result == PostTransactionRound.FPMM_REMOVE_LIQUIDITY_TX_DONE_PAYLOAD

    def test_get_payload_ct_redeem_tokens_submitter(self) -> None:
        """Test get_payload when tx_submitter is CtRedeemTokensRound."""
        self.behaviour.synchronized_data.settled_tx_hash = "0xabc"
        self.behaviour.synchronized_data.tx_submitter = (
            CtRedeemTokensRound.auto_round_id()
        )

        gen = self.behaviour.get_payload()
        result = _exhaust_gen(gen)

        assert result == PostTransactionRound.CT_REDEEM_TOKENS_TX_DONE_PAYLOAD

    def test_get_payload_realitio_withdraw_bonds_submitter(self) -> None:
        """Test get_payload when tx_submitter is RealitioWithdrawBondsRound."""
        self.behaviour.synchronized_data.settled_tx_hash = "0xabc"
        self.behaviour.synchronized_data.tx_submitter = (
        ...
        assert result == PostTransactionRound.REALITIO_WITHDRAW_BONDS_TX_DONE_PAYLOAD
    ...
    def test_get_payload_funds_forwarder_submitter(self) -> None:
        ...
        assert result == PostTransactionRound.FUNDS_FORWARDER_TX_DONE_PAYLOAD
