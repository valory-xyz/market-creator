...
def behaviour(monkeypatch):
    ...
    monkeypatch.setattr(DummyPostTransactionBehaviour, "context", 
...
@pytest.mark.parametrize("test_case, tx_sender, settled_tx_hash, is_approved_data_set, market_id, expected_payload", [
    ("no_settled_tx_hash", None, None, True, "market_id", PostTransactionRound.DONE_PAYLOAD),
    ("mech_request_sender", MechRequestStates.MechRequestRound.auto_round_id(), "tx_hash", True, "market_id", PostTransactionRound.MECH_REQUEST_DONE_PAYLOAD),
    ("redeem_bond_sender", RedeemBondRound.auto_round_id(), "tx_hash", True, "market_id", PostTransactionRound.REDEEM_BOND_DONE_PAYLOAD),
    ("deposit_dai_sender", DepositDaiRound.auto_round_id(), "tx_hash", True, "market_id", PostTransactionRound.DEPOSIT_DAI_DONE_PAYLOAD),
    ("answer_question_sender", AnswerQuestionsRound.auto_round_id(), "tx_hash", True, "market_id", PostTransactionRound.ANSWER_QUESTION_DONE_PAYLOAD),
    ("remove_funding_sender", RemoveFundingRound.auto_round_id(), "tx_hash", True, "market_id", PostTransactionRound.REMOVE_FUNDING_DONE_PAYLOAD),
    ("no_question_data_set", None, "tx_hash", False, "market_id", PostTransactionRound.DONE_PAYLOAD),
    ("no_market_id", None, "tx_hash", True, None, PostTransactionRound.DONE_PAYLOAD),
    ("non_prepare_tx_sender", "OtherRound", "tx_hash", True, "market_id", PostTransactionRound.DONE_PAYLOAD),
])
def test_get_payload_various_cases(behaviour, monkeypatch, test_case, tx_sender, settled_tx_hash, is_approved_data_set, market_id, expected_payload):
...
def test_get_payload_prepare_transaction_sender(behaviour, monkeypatch):
    ...
    handle_market_creation_mock = MagicMock(side_effect=[gen_side_effect(PostTransactionRound.DONE_PAYLOAD)])
    monkeypatch.setattr(behaviour, "_handle_market_creation", handle_market_creation_mock)
    ...
    def mock_get_payload():
        ...
        behaviour.context.logger.info(
            f"Handling settled tx hash {tx_hash}. "
...
def test_handle_market_creation_get_fpmm_id_fails(behaviour, monkeypatch):
    ...
    # Mock dependencies
    monkeypatch.setattr(
        behaviour, "_get_fpmm_id",
        MagicMock(side_effect=[gen_side_effect(None)])
    )
    
    monkeypatch.setattr(
        behaviour, "_mark_market_as_done",
        MagicMock()
    )
...
def test_handle_market_creation_mark_market_fails(behaviour, monkeypatch):
    ...
    monkeypatch.setattr(
        behaviour, "_get_fpmm_id",
        MagicMock(side_effect=[gen_side_effect(fpmm_id)])
    )
...
def test_get_fpmm_id_success(behaviour, monkeypatch):
    ...
    monkeypatch.setattr(
        behaviour, "get_contract_api_response",
    ...
    assert call_args[1]['contract_address'] == behaviour.params.fpmm_deterministic_factory_contract
...
def test_mark_market_as_done_success(behaviour, monkeypatch):
    ...
    get_http_response_mock = MagicMock(side_effect=[
...
def test_mark_market_as_done_second_update_fails(behaviour, monkeypatch):
    ...
    get_http_response_mock = MagicMock(side_effect=[
        gen_side_effect(update_response),
        gen_side_effect(failed_response)
    ])
    monkeypatch.setattr(behaviour, "get_http_response", get_http_response_mock)
