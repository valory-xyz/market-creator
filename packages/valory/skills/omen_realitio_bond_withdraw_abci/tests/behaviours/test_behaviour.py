...
"""Tests for RealitioBondWithdrawBehaviour."""
...
class TestRealitioBondWithdrawBehaviour:
    """Tests for RealitioBondWithdrawBehaviour."""
    ...
    def test_build_claim_txs_batch_size_limit(self) -> None:
        """_build_claim_txs stops after reaching realitio_bond_withdraw_batch_size."""
        self.behaviour.context.params.realitio_bond_withdraw_batch_size = 1
        responses = [
    ...
    def test_get_claimable_responses_query_uses_batch_size(self) -> None:
        ...
        self.behaviour.context.params.realitio_bond_withdraw_batch_size = 7
        captured_query: Dict[str, str] = {}
...
class TestEquivalenceWithBaseline:
    """Equivalence assertion vs. baseline ClaimBondsBehaviour.

    This skill produces the same per-tx outputs as the baseline
    ClaimBondsBehaviour for matched mocked inputs.

    Per the relaxed equivalence invariant in the overview plan, the
    intent is functional equivalence — same target contract, same call,
    same on-chain effect — not byte-identical hex output. The baseline's
    per-tx helpers (_get_claim_params, _simulate_claim, _build_claim_tx,
    _maybe_build_withdraw_tx) are lifted verbatim, so they MUST produce
    the same tx dicts for the same input. The only architectural
    difference is that this skill wraps the txs in its own multisend
    (single-skill scope) instead of contributing to a shared accumulator.
    """

    def setup_method(self) -> None:
        """Set up two behaviours sharing the same mocked context."""
        context_mock = MagicMock()
        context_mock.logger = MagicMock()
        context_mock.params = MagicMock()
        context_mock.params.realitio_contract = "0xRealitio"
        ...
        self.new_behaviour: Any = RealitioBondWithdrawBehaviour(
            name="new", skill_context=context_mock
        )

    def test_assemble_claim_params_matches_baseline_algorithm(self) -> None:
        """The promoted assemble_claim_params matches the baseline.

        This is the most critical equivalence assertion: the calldata-shape
        algorithm is verbatim from the baseline and must produce identical
        output for any input.
        """
        from packages.valory.skills.omen_funds_recoverer_abci.behaviours.claim_bonds import (  # noqa: E501
            _assemble_claim_params as baseline_assemble,
        )
        from packages.valory.skills.omen_realitio_bond_withdraw_abci.behaviours.base import (  # noqa: E501
            assemble_claim_params as new_assemble,
        )

        # Build a 3-entry chronological history.
        answered = [
            {
                "args": {
                    "user": "0xUser1",
            ...
            },
            {
                "args": {
                    ...
                    "bond": 400,
                    "answer": b"\x00" * 32,
                    "history_hash": b"\x33" * 32,
                }
            },
        ]
        baseline_result = baseline_assemble(answered)
        new_result = new_assemble(answered)
        assert baseline_result == new_result

    def test_build_claim_tx_matches_baseline_for_same_input(self) -> None:
        """_build_claim_tx matches the baseline ClaimBondsBehaviour output.

        Produces an equivalent tx dict to the baseline
        ClaimBondsBehaviour._build_claim_tx for the same mocked contract
        response.
        """
        from packages.valory.skills.omen_funds_recoverer_abci.behaviours.claim_bonds import (  # noqa: E501
            ClaimBondsBehaviour,
        ...
        baseline_context = MagicMock()
        baseline_context.logger = MagicMock()
        baseline_context.params = MagicMock()
        baseline_context.params.realitio_contract = "0xRealitio"
        baseline_context.params.claim_bonds_batch_size = 10
        baseline_context.params.min_balance_withdraw_realitio = 10000000000000000000
        baseline_context.state.round_sequence = MagicMock()
        baseline_context.state.synchronized_data = MagicMock()
        baseline_context.state.synchronized_data.safe_contract_address = "0xSafe"
        baseline_context.benchmark_tool = MagicMock()
        baseline_context.agent_address = "0xAgent"
        baseline_context.realitio_subgraph = MagicMock()
        baseline_behaviour: Any = ClaimBondsBehaviour(
            name="baseline", skill_context=baseline_context
        )

        # Mock the same response from both contract calls.
        resp = make_contract_state_response({"data": "0xClaimData"})
        ...
        with patch.object(
            ...
            baseline_gen = baseline_behaviour._build_claim_tx(
                b"\x01" * 32, [{"p": "v"}]
            )
            baseline_result = exhaust_gen(baseline_gen)
        ...
        with patch.object(
            ...
            new_gen = self.new_behaviour._build_claim_tx(b"\x01" * 32, [{"p": "v"}])
            new_result = exhaust_gen(new_gen)

        # Both should produce the same {to, data, value} dict.
        assert baseline_result == new_result
