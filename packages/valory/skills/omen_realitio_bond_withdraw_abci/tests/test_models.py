...
from packages.valory.skills.omen_realitio_bond_withdraw_abci.models import (
    RealitioBondWithdrawParams,
    RealitioSubgraph,
    SharedState,
)
...
def test_import() -> None:
    ...
    assert RealitioBondWithdrawParams is not None
...
def test_params_init_carries_required_params() -> None:
    ...
    ``realitio_bond_withdraw_batch_size`` and that the deleted
    ...
    kwargs: dict = {
        "realitio_bond_withdraw_batch_size": 10,
    ...
    instance = RealitioBondWithdrawParams.__new__(RealitioBondWithdrawParams)
    with (
        patch.object(RealitioBondWithdrawParams, "_ensure", new=fake_ensure),
        patch.object(BaseParams, "__init__", return_value=None),
    ):
        RealitioBondWithdrawParams.__init__(instance, **kwargs)
    assert instance.realitio_bond_withdraw_batch_size == 10
    assert instance.min_balance_withdraw_realitio == 10**19
...
def test_shared_state_init() -> None:
    ...
    assert SharedState.abci_app_cls is OmenRealitioBondWithdrawAbciApp
