...
"""Tests for CtRedeemBehaviour."""

# pylint: disable=protected-access,too-many-public-methods,too-many-locals
...
from packages.valory.skills.omen_ct_redeem_abci.behaviours.base import ETHER_VALUE
...
def _build_behaviour() -> Any:
    """Build a CtRedeemBehaviour with a fully mocked context."""
    ...
    return CtRedeemBehaviour(name="test", skill_context=context_mock)


class TestCtRedeemBehaviourSimpleHelpers:
    """Tests for the small static and contract-call helpers on CtRedeemBehaviour."""
    ...
    def test_has_winning_position_true(self) -> None:
        ...
        payouts = ["1", "0"]
        held_index_sets = {1, 2}
        assert CtRedeemBehaviour._has_winning_position(payouts, held_index_sets)
...
class TestCtRedeemBehaviourBuildRedeemTxs:
    ...
    def test_build_redeem_txs_respects_batch_size(self) -> None:
        ...
        self.behaviour.context.params.ct_redeem_batch_size = 1
        markets = [
            {
                "address": f"0xM{i}",
...
class TestCtRedeemBehaviourRedeemableMarkets:
...
class TestCtRedeemBehaviourPrepareMultisend:
...
class TestCtRedeemBehaviourAsyncAct:
...
class TestEquivalenceWithBaseline:
    """Equivalence assertion against the baseline RedeemPositionsBehaviour.

    Locks the per-tx output of the new pipeline to be functionally
    equivalent to the baseline tx-list construction (per the relaxed
    equivalence invariant in omen_funds_recoverer_split.md).
    """

    def setup_method(self) -> None:
        """Set up the new-skill behaviour with deterministic fixtures."""
        self.behaviour: Any = _build_behaviour()

    @staticmethod
    def _fixture_held() -> dict:
        """Build a deterministic held-positions dict."""
        return {"0xcond1": {1}}

    @staticmethod
    def _fixture_markets() -> list:
        """Build a deterministic markets list with one winning market."""
        return [
            {
                "address": "0xM1",
                "condition_id": "0xcond1",
    ...
    @staticmethod
    def _fixture_resolve_tx() -> dict:
        """Build a deterministic resolve tx dict."""
        return {"to": "0xRealitioProxy", "data": "0xresolve", "value": 0}

    @staticmethod
    def _fixture_redeem_tx() -> dict:
        """Build a deterministic redeem tx dict."""
        return {"to": "0xConditionalTokens", "data": "0xredeem", "value": 0}

    def test_tx_output_matches_baseline_already_resolved(self) -> None:
        """The new pipeline produces the same tx list as the baseline when condition is resolved.

        Baseline ``RedeemPositionsBehaviour._build_redeem_positions_txs``
        for an already-resolved single winning condition produces a single
        ``redeem`` tx (no resolve prepended). The new
        ``CtRedeemBehaviour._build_redeem_txs`` must produce the
        functionally equivalent ``[redeem_tx]``.
        """
        held = self._fixture_held()
        markets = self._fixture_markets()
        redeem_tx = self._fixture_redeem_tx()
        with (
            patch.object(self.behaviour, "_get_held_positions", new=make_gen(held)),
            patch.object(
                self.behaviour, "_get_markets_for_conditions", new=make_gen(markets)
            ...
            redeemable_gen = self.behaviour._get_redeemable_markets(held)
            redeemable = exhaust_gen(redeemable_gen)
            txs_gen = self.behaviour._build_redeem_txs(redeemable)
            new_txs = exhaust_gen(txs_gen)

        # Baseline expectation: a single redeem tx, no resolve prepended.
        baseline_txs = [redeem_tx]
        assert new_txs == baseline_txs

    def test_tx_output_matches_baseline_with_resolve(self) -> None:
        """The new pipeline produces the same tx list as the baseline when resolve must be prepended.

        Baseline behaviour for an unresolved winning condition produces
        ``[resolve_tx, redeem_tx]`` (resolve first, redeem second). The
        new pipeline must produce the same two-element list in the same
        order.
        """
        held = self._fixture_held()
        markets = self._fixture_markets()
        resolve_tx = self._fixture_resolve_tx()
        redeem_tx = self._fixture_redeem_tx()
        with (
            patch.object(self.behaviour, "_get_held_positions", new=make_gen(held)),
            patch.object(
        ...
        baseline_txs = [resolve_tx, redeem_tx]
        assert new_txs == baseline_txs
